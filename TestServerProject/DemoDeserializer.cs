// --------------------------------------------------------------------------------------------------------------------
// <copyright file="DemoDeserializer.cs" company="OccupOS">
                ...
                foreach (string sensor in sensors)
                {
                    ...
                    foreach (string reading in readings)
                    {
                        ...
                        if (!id.Contains("1."))
                        {
                            ...
                            {
                                case "a":
                                    data.ReadTime = Convert.ToDateTime(GetDataElement(reading));
                                    break;
                                case "b":
                                    data.PollTime = Convert.ToDateTime(GetDataElement(reading));
                                    break;
                                case "0":
                                    int.TryParse(GetDataElement(reading), istyle, culture, out inum);
                                    data.EntityCount = inum;
                                    break;
                                case "2":
                                    float.TryParse(GetDataElement(reading), fstyle, culture, out fnum);
                                    data.SoundDb = fnum;
                                    break;
                                case "3":
                                    float.TryParse(GetDataElement(reading), fstyle, culture, out fnum);
                                    data.AnalogLight = fnum;
                                    break;
                                case "4":
                                    float.TryParse(GetDataElement(reading), fstyle, culture, out fnum);
                                    data.VibrationHz = fnum;
                                    break;
                                case "5":
                                    float.TryParse(GetDataElement(reading), fstyle, culture, out fnum);
                                    data.Humidity = fnum;
